...
class PokemonListViewModelTests: XCTestCase {
    ...
    func testFetchPokemonList_Success() async {
        ...
        mockNetwork.mockPokemonSummaries = expectedPokemons
    ...
    func testFetchPokemonList_Failure() async {
        ...
        XCTAssertEqual(viewModel.errorMessage, "Error fetching Pokémon list: The operation couldn’t be completed. (PokemonApp.NetworkError error 1.)")
    }
    ...
    func testLoadNextPage() async {
        ...
        mockNetwork.mockPokemonSummaries = firstPagePokemons
        await viewModel.fetchPokemonList()

        XCTAssertEqual(viewModel.pokemons.count, 1)
        XCTAssertEqual(viewModel.pokemons.first?.name, "Charmander")

        mockNetwork.mockPokemonSummaries = secondPagePokemons
